...
Methods:
  doctor()                       -> {checks: [...]}
  list_runs()                    -> [run_id, ...]
  get_run(run_id)                -> {record, summary, patch, files}
  get_events(run_id)             -> [event, ...]
  start_run(task)                -> {run_id, summary, patch, files}
  apply(run_id, dry_run?)        -> {ok, files, stderr, dry_run}
...
from gemcoder.managed import DEFAULT_MODEL, ManagedAgentClient, ManagedAgentError
...
def _start_run(root: Path, task: str) -> dict[str, Any]:
    ...
    payload = json.dumps({"summary": result.summary, "patch": result.patch}, indent=2)
    store.write_artifact(run_id, "managed-result.json", payload + "\n")
    if result.patch:
        store.write_artifact(run_id, "patch.diff", result.patch)
        store.append(run_id, "patch.received")
    else:
        store.append(run_id, "patch.empty")
    return {"run_id": run_id, "summary": result.summary, "patch": result.patch}
...
def _verify(root: Path, run_id: str | None = None) -> list[dict[str, Any]]:
    config = load_config(root)
    store = RunStore(root)
    selected = run_id or (store.list_runs()[-1] if store.list_runs() else "manual")
    ...
    for r in results:
        store.append(
            ...
            {"command": r.command, "status": "pass" if r.returncode == 0 else "fail"},
        )
...
def _info(root: Path) -> dict[str, Any]:
    config = load_config(root)
    return {
        "model": config.managed_agent.agent_id or DEFAULT_MODEL,
...
def handle_request(req: dict[str, Any], dispatch: dict[str, Callable[..., Any]]) -> dict[str, Any]:
